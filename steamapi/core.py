...
# This is the template string for the api_list module that gets autogenerated from steam's api spec call
API_SPEC_TEMPLATE = """#AUTOGENERATED! Do not edit manually
APILIST={}
"""
WEB_API_DOMAIN = "api.steampowered.com"
...
class APICall(object):
    ...
    def __call__(self, method=GET, **kwargs):
        ...
        if self._is_registered is False and type(self._parent) is not APIInterface:
            self._parent._register(self)
...
class APIInterface(object):
    def __init__(self, api_key=None, autopopulate=False, strict=False,
                 api_domain=WEB_API_DOMAIN, api_protocol="http", settings=None,
                 validate_key=False, refresh=False):
        """
        ...
        :type validate_key: bool
        :param refresh: pulls new steam API spec
        :type refresh: bool
        :return:
        """
        ...
        if autopopulate is True:
            ...
            try:
                ...
                if api_domain == WEB_API_DOMAIN: # autopopulate is really only defined for the web api domain
                    if refresh: # if refresh is set, then we should pull a new api spec from steam. If the api spec has changed, please submit a pull request.
                        self._refresh_interfaces()
                    else: # else just use the api spec that we have saved
                        response = APIResponse(api_list.APILIST)
    ...
    # Generic populate that can be populated with steam web spec response or some user-defined object
    def _populate(self, api_definition):
    ...
    # Get new api_definitions from steam web api
    def _refresh_interfaces(self):
        ...
        self._populate(api_definition)

        # saves api_definition as python module
        api_spec = API_SPEC_TEMPLATE.format(pprint.pformat(api_definition.__dict__))
        dirname = os.path.dirname(__file__)
        filename = os.path.join(dirname, 'api_list.py')
        api_spec_file = open(filename, 'w+')
        api_spec_file.write(api_spec)
...
class APIResponse(object):
    ...
    def __dict__(self):
        ...
        # we recursively convert all the inner APIResponses into dicts
        for k in self._real_dictionary:
            if isinstance(self._real_dictionary[k], APIResponse):
                new_dict[k] = self._real_dictionary[k].__dict__
            elif isinstance(self._real_dictionary[k], list):
                new_dict[k] = []
                for item in self._real_dictionary[k]:
                    new_dict[k].append(item.__dict__)
            else:
                new_dict[k] = self._real_dictionary[k]
        return new_dict
